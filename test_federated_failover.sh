...
cleanup() {
    ...
    rm -f fed1.log fed2.log fed3.log mock_db.log
    exit
}
...
java -jar "$FED_JAR" 9000 fed-9000 http://localhost:9001 http://localhost:9002 -bootstrap > fed1.log 2>&1 &
PID1=$!
echo "Fed 1 started (PID: $PID1) on port 9000 with -bootstrap"

# Start Fed 2 on 9001
java -jar "$FED_JAR" 9001 fed-9001 http://localhost:9000 http://localhost:9002 > fed2.log 2>&1 &
PID2=$!
echo "Fed 2 started (PID: $PID2) on port 9001"

# Start Fed 3 on 9002
java -jar "$FED_JAR" 9002 fed-9002 http://localhost:9000 http://localhost:9001 > fed3.log 2>&1 &
PID3=$!
